...
- (void) openAudioURL:(NSURL *)url
{
    // get rid of any previous player
    ...
    // get sample data for the visualisation
	[wsp release];
    ...
    // set up Dirac
    NSError *error = nil;
    mDiracAudioPlayer = [[DiracAudioPlayer alloc] initWithContentsOfURL:url channels:1 error:&error];		// LE only supports 1 channel!
...
- (void) cueAudio
{
	if(wsp) {
		if(mDiracAudioPlayer == nil) {
            ...
            // if player is not playing then start playback
            if(!mDiracAudioPlayer.playing) {
                [self startAudio];
                [mDiracAudioPlayer play];
				[self setInfoString:@"Playing"];
			} else {
                // its playing so lets stop it
                [mDiracAudioPlayer stop];
                [self _stopTimer]; //stop our timer
...
-(void)_timerFired {
    if (mDiracAudioPlayer != nil){
        double duration = mDiracAudioPlayer.fileDuration;
        double currentTime = mDiracAudioPlayer.currentTime;
        /*
       ...
       // NSLog(@"play progress, %f", playProgress);
        [self setNeedsDisplay];
    }
}
...
- (void) startAudio
{
	if(wsp.status == LOADED) {
        if (_timer == nil) {
            //set up a timer to run at 0.1 sec intervals
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    //check if we tapped inside the waveform view
    if(CGRectContainsPoint(wr,local_point) && mDiracAudioPlayer != nil) {
		CGFloat x = local_point.x - wr.origin.x;
		double sel = x / wr.size.width;
        NSLog(@"SEL : %f", sel);
		double duration = mDiracAudioPlayer.fileDuration;
        NSLog(@"Duration from Dirac: %f", mDiracAudioPlayer.currentTime);
        
		double timeSelected = duration * sel;
		//CMTime tm = CMTimeMakeWithSeconds(timeSelected, NSEC_PER_SEC);
        [mDiracAudioPlayer setCurrentTime:timeSelected];
		//[player seekToTime:tm];
		NSLog(@"Seek to time : %f",timeSelected);
	}
    /*
...
-(void)changeDuration:(float)duration {
   [mDiracAudioPlayer changeDuration:duration];
}
...
- (void)drawRect:(CGRect)dirtyRect
{
	...
	*/
	...
	[self drawRoundRect:waveRect fillColor:white strokeColor:darkgray radius:4.0 lineWidht:2.0];
	/*
    // playback time / status container
	...
	*/
	...
	if(sampleLength > 0) {
		...
		// gauge draw
        // +++++++ I think this is the shading showing progress of the track as its playing ++++
		if(playProgress > 0.0) {
