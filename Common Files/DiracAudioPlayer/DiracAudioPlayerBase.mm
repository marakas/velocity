...
{    
    ...
    Self.mFramePositionInInputFile += inNumberFrames; // keep our position up to date
    return noErr;
}
...
- (void) setCurrentTime:(NSTimeInterval)time
{
	...
	if (mReader) {
		...
		if (duration>0 && seekPos < duration) {
            ...
            mFramePositionInInputFile = seekPos;
...
-(NSTimeInterval)currentTime
{
    ...
    //return (NSTimeInterval)mLastResetPositionInFile / mSampleRate;
	
    // this was the code used in version 1.0:
    //return (NSTimeInterval)mTotalFramesPlayed / mSampleRate;
    //return (NSTimeInterval)mTotalFramesPlayed + mLastResetPositionInFile / mSampleRate;
}
